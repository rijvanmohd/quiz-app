...
class Level(models.Model):
    ...
    question_level = models.PositiveIntegerField(choices=LEVEL_OPTIONS, default=LEVEL_EASY)
    marks = models.PositiveIntegerField(default=0)

    def __str__(self):
        return self.get_question_level_display()
...
def set_default_quiz(sender, instance, created,**kwargs):
    ...
    quiz.update(questions_count=instance.question_set.filter(quiz=instance.pk).count(),\
        total_point=instance.question_set.filter(quiz=instance.pk).aggregate(Sum('level__marks'))['level__marks__sum'])
