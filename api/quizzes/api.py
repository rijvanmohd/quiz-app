...
from .models import Answer, Question, Quiz, QuizTaker, UserAnswer
    ...
    def get(self, *args, **kwargs):
        ...
        if created:
            for question in Question.objects.filter(quiz=quiz):
                UserAnswer.objects.create(quiztaker=obj, question=question)
        else:
...
class SaveUsersAnswer(generics.UpdateAPIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        obj,_ = UserAnswer.objects.get_or_create(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if answer_id is not None:
            ...
            obj = get_object_or_404(
                UserAnswer, quiztaker=quiztaker, question=question)
            obj.answer = answer
            obj.save()
        ...
        for users_answer in UserAnswer.objects.filter(quiztaker=quiztaker):
            answer = Answer.objects.get(
                question=users_answer.question, is_correct=True)
            print(answer)
            print(users_answer.answer)
            if users_answer.answer == answer:
                quiztaker.correct_answers += 1
                score += answer.question.level.marks
